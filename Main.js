...
let arrRules2=[
[
    "This section gives answers to technical, detailed questions. You do not need to read it when first learning.", "Rule Conflicts", "Public and Private Information", "Negotiation and Deals", "Game Structure", "Pieces"
],
[
...
[
    [""],
    ["", "Step 1: Assign Factions and Starting Player", "Step 2: Place Score Markers", "Step 3: Draw Starting Hands", "Step 4: Place Ruins", "Step 5: Form Items Supply", "Step 6: Gather Other Pieces", "Step 7: Set Up Factions"]
]];
let arrRules4 = [
    [
        [""],
        ...
        ["", "Players may discuss the game and make agreements, but they are non-binding.", "Players may only give cards to each other if explicitly instructed."],
    ...
    [
        ...
        ["", "During your Daylight, if you have at least 10 victory points, you may activate a dominance card in your hand by placing it in your play area. Remove your score marker from the score track. You can no longer score victory points. For Mouse, Rabbit, or Fox Dominance, you win the game immediately if you rule three clearings of the suit matching the activated dominance card at the start of your Birdsong. For Bird Dominance, you win the game immediately if you rule two clearings in opposite corners at the start of your Birdsong.", "An activated dominance card does not count against your hand limit, and it cannot be removed from play. You cannot replace an activated dominance card with a different one.", " A dominance card can be spent for its suit. Anytime a dominance card would be placed in the discard pile, instead place it near the map to show it is available to be taken.", "During your Daylight, you can take an available dominance card into your hand by spending a card of matching suit. You cannot treat an available bird dominance card as a non-bird suit."]
    ],
    [
        [""],
...
function showRule(number1,number2,number3,number4){
    for(let i=1; i<arrRules1.length+1; i++){
        for(let o=0; o<arrRules2[i-1].length; o++){
            for(let p=0; p<arrRules3[i-1][o].length; p++){
                for(let l=0; l<3;l++){
                    let CurrentElement=document.getElementById((i)+""+(o)+""+(p)+""+l)
                    if(number4!=1){
                        document.getElementById("div2").style.backgroundColor="rgb(101, 25, 25)";
                    }
                    else{
                       document.getElementById("div2").style.backgroundColor="white";
                    }
                    if(number1==i){
                        document.getElementById("secDescription1").textContent=arrRules2[i-1][0];
                        if(arrRules2[i-1][0]==""){
                                document.getElementById("secDescription1").style.padding="0px";
                            }
                            else{
                                document.getElementById("secDescription1").style.padding="5px";
                            }
                        if(o==number2){
                            document.getElementById("secDescription2").textContent=arrRules3[i-1][o][0]
                            if(arrRules3[i-1][o][0]==""){
                                document.getElementById("secDescription2").style.padding="0px";
                            }
                            else{
                                document.getElementById("secDescription2").style.padding="5px";
                            }
                        }
                    }
...
if(CurrentPage=="Main_Page.html"){
    for(let i=0; i<arrRules1.length; i++){
        ...
        astuff.setAttribute("href", "Rules.html#"+(i+1)+"OOO");
    ...
    for(let i=1; i<(arrRules1.length)+1; i++){
        ...
        Subs1.textContent=i+".0 ";
        let Div1=document.createElement("div");
        Div1.textContent=arrRules1[i-1];
        Subs1.setAttribute("id", i+"OOO");
        Subs1.setAttribute("class", ["blue", "orange","blue", "orange","blue"][i-1]);
        Subs1.setAttribute("onclick", "showRule("+i+",0,0,0)");
        document.getElementById("bigSection").appendChild(Subs1);
        document.getElementById(i+"OOO").appendChild(Div1);
        for(let o=1; o<arrRules2[i-1].length;o++){
            let Subs2=document.createElement("button");
            Subs2.textContent=i+"."+o+": "+arrRules2[i-1][o];
            Subs2.setAttribute("id", i+""+o+""+0+""+0);
            Subs2.setAttribute("onclick", "showRule("+i+","+o+",0,0)");
            document.getElementById("Rules"+i).appendChild(Subs2);
            for(let p=1; p<arrRules3[i-1][o].length;p++){
                let Subs3=document.createElement("button");
                Subs3.textContent=i+"."+o+"."+p+": "+arrRules3[i-1][o][p];
                Subs3.setAttribute("id", i+""+o+""+p+""+0);
                Subs3.setAttribute("onclick", "showRule("+i+","+o+","+p+",1)");
                document.getElementById("div1").appendChild(Subs3);
                let Subs4=document.createElement("div");
                Subs4.textContent=arrRules4[i-1][o][p];
                Subs4.setAttribute("id", i+""+o+""+p+""+1);
                document.getElementById("div2").appendChild(Subs4);
            }
        }
    }
    showRule(CSec1,CSec2,CSec3,CSec4)
}